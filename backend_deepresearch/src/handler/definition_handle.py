...
def handle_node_update(node: str, update: Dict[str, Any]) -> Iterable[Event]:
    if node == "clarify_first":
        ...
        if update.get("need_addition_questions") is True:
            ...
            # 1-1. 다시 입력하라
            if len(addition_questions) == 0:
                if qna_ment:
                    yield ("llm", {"type": "error_messages", "content": qna_ment})
                return

            # 1-2. 추가질문
            if qna_ment:
                yield ("llm", {"type": "qna_ment", "content": qna_ment})
            for add_que in addition_questions:
                yield ("llm", {"type": "addition_questions", "questions": add_que})
            return
